    ...
    for (var j = 0; j < _fv.length; j++) {
     ...
     if (m = comp.match(/^\s*(>|<|[><!=]=)\s*([\d\.\-]+|field:(.+))\s*$/)) {
      ...
      if (m[3]) {
        ...
        val2 = 1 * v_get_form_value(form[field2]);
      }
      hold = (m[1] == '>' ) ? (copy >  val2)   : (m[1] == '<' ) ? (copy <  val2)
           : (m[1] == '>=') ? (copy >= val2)   : (m[1] == '<=') ? (copy <= val2)
           : (m[1] == '!=') ? (copy != val2)   : (m[1] == '==') ? (copy == val2)  : false;
     } else if (m = comp.match(/^\s*(eq|ne|gt|ge|lt|le)\s+(field:(.+)|.+?)\s*$/)) {
      var val2 = m[2];
      if (m[3]) {
        var field2 = m[3].replace(/\$(\d+)/g, function (all, N) {
          return (typeof(ifs_match) != 'object' || typeof(ifs_match[N]) == 'undefined') ? '' : ifs_match[N];
        });
        val2 = v_get_form_value(form[field2]); if (typeof(val2) == 'undefined') val2 = '';
      } else if (val2.match(/^\"/)) val2 = val2.replace(/^"(.*)"$/,'$1');
        else if (val2.match(/^\'/)) val2 = val2.replace(/^'(.*)'$/,'$1');
      hold = (m[1] == 'gt') ? (copy >  val2)   : (m[1] == 'lt') ? (copy <  val2)
