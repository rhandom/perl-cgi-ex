// Copyright 2007 - Paul Seamons - $Revision: 1.54 $
...
function eob_as_hash (extra2) {
 ...
 for (var i = 0; i < errors.length; i++) {
  ...
  if (ret[field].length == 0 || ! extra2.first_only) ret[field].push(text);
 }
...
function v_el_attach (el, field_val, form, val_hash) {
 ...
 var func = function () {
  ...
  if (! e || e == '') {
   ...
   v_inline_error_clear(field+'_error', val_hash, form);
   return;
  }
  e = new ValidateError(e, {});
  e = e.as_hash({first_only:(val_hash['group first_only']?1:0)});
  for (var key in e) {
   if (key == 'extend') continue; // Protoype Array()
...
function v_inline_error_clear (key, val_hash, form) {
   ...
   if (typeof(f) == 'function') f(key, val_hash, form);
}
...
function v_inline_error_set (key, val, val_hash, form) {
   ...
   if (typeof(f) == 'function') f(key, val, val_hash, form);
}
