// Copyright 2007 - Paul Seamons - $Revision: 1.67 $
...
function v_validate (form, val_hash) {
 ...
 if (v_if && ! v_check_conditional(form, v_if)) return;
...
function v_check_conditional (form, ifs, ifs_match) {
 var is_ok = 1;
 for (var i = 0; i < ifs.length; i++) {
  if (typeof(ifs[i]) == 'function') {
   ...
   var err = v_validate_buddy(form, field, ifs[i]);
...
function v_validate_buddy (form, field, field_val, ifs_match) {
 var errors = [];
 ...
 if (m = field.match(/^(!\s*|)m([^\s\w])(.*)\2([eigsmx]*)$/)) {
  ...
  for (var i = 0; i < form.elements.length; i++) {
   ...
   if ( (not && ! (m = _field.match(reg))) || (m = _field.match(reg))) {
    var err = v_validate_buddy(form, _field, field_val, m);
 ...
 for (var i = 0; i < tests.length; i++) {
  ...
  var ret = v_check_conditional(form, ifs, ifs_match);
  if (ret) needs_val++;
 }
 ...
 if (! is_required) {
  ...
  for (var i = 0; i < tests.length; i++) {
   var ifs = field_val[tests[i]];
   if (! v_check_conditional(form, ifs, ifs_match)) continue;
   is_required = tests[i];
   break;
  }
 }
...
function v_check_type (value, type, field, form) {
 ...
 if (type == 'EMAIL') {
  ...
  if (m[2] == 'name') {
   if (! m[1].match(/^([a-z0-9][a-z0-9\-]{1,62}\.){2}$/)) return 0;
  } else
   if (! m[1].match(/^([a-z0-9][a-z0-9\-]{1,62}\.)+$/)) return 0;
...
function v_set_form_value (el, values, form) {
 ...
 if (el.length && type != 'select-one') {
  for (var i = 0; i < el.length; i++)
    v_set_form_value(el[i], (i < values.length ? [values[i]] : ['']));
  return;
 }
 if (! type) return;
 ...
 if (type == 'checkbox' || type == 'radio') return el.checked = (el.value == values[0]) ? true : false;
 if (type == 'file') return;
