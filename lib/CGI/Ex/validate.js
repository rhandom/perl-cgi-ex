...
function eob_get_val (key, extra1, extra2, default) {
...
function eob_as_string (extra2) {
  ...
  var joiner = eob_get_val('as_string_join',   extra2, this.extra, '<br />');
  var header = eob_get_val('as_string_header', extra2, this.extra, '');
  var footer = eob_get_val('as_string_footer', extra2, this.extra, '');
...
function eob_as_array () {
  ...
  var title = eob_get_val('as_array_title', extra2, this.extra, 'Please correct the following items:');
  ...
  if (title) has_headings = 1;
    ...
    for (var i = 0; i < this.errors.length; i ++) {
      if (typeof(this.errors[i]) != 'string') continue;
      has_headings = 1;
      break;
    }
  }

  var prefix = eob_get_val('as_array_prefix', extra2, this.extra, has_headings ? '  ' : '');

  /// get the array ready
  var arr = new Array();
  if (title.length) arr[arr.length] = title;

  /// add the errors
  var found = new Array();
  for (var i = 0; i < this.errors.length; i ++) {
    if (typeof(this.errors[i]) == 'string') {
      arr[arr.length] = this.errors[i];
      found = new Array();
    } else {
      var text = this.get_error_text(this.errors[i]);
...
function eob_as_hash () {
  var suffix = eob_get_val('as_hash_suffix', extra2, this.extra, '_error');
  var joiner = eob_get_val('as_hash_join',   extra2, this.extra, '<br />');
  ...
  for (var i = 0; i < this.errors.length; i ++) {
    ...
    var field     = this.errors[i][0];
    var type      = this.errors[i][1];
    var field_val = this.errors[i][2];
    var ifs_match = this.errors[i][3];
  ...
  if (joiner) {
    var header = eob_get_val('as_hash_header', extra2, this.extra, '');
    var footer = eob_get_val('as_hash_footer', extra2, this.extra, '');
    for (var key in ret) {
