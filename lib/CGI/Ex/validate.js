// Copyright 2007 - Paul Seamons - $Revision: 1.53 $
...
function v_inline_error_clear (key) {
   var el = document.getElementById(key);
   if (el) el.innerHTML = '';
   delete(v_did_inline[key]);
}

function v_inline_error_set (key, val) {
   var el = document.getElementById(key);
...
document.validate = function (form, val_hash) {
 ...
 if (! err_obj.extra.no_inline) {
  ...
  for (var key in hash) {
   if (key == 'extend') continue; // Protoype Array()
   v_inline_error_set(key, hash[key]);
  }
 }
...
document.check_form = function (form, val_hash) {
 ...
 if (types.change || types.blur) {
  ...
  for (var i = 0; i < clean.fields.length; i++) {
   ...
   v_el_attach(el, clean.fields[i], form);
  }
 }
...
function v_el_attach (el, field_val, form) {
 if (! el.type) return;
 if (el.type.match(/(password|text|textarea)/)) {
  el.onblur = function () {
