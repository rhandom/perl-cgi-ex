// Copyright 2007 - Paul Seamons - $Revision: 1.56 $
...
function v_field_order (field_val) {
 var o = [];
 for (var k in field_val) if (k != 'extend') o.push(k);
 return o.sort();
}
...
function v_validate_buddy (form, field, field_val, N_level, ifs_match) {
 ...
 for (var n = 0; n < values.length; n++) {
  ...
  for (var i = 0; i < tests.length; i++) {
   var a = field_val[tests[i]];
   var is_found = 0;
   for (var j = 0; j < a.length; j++) if (value == a[j]) { is_found = 1; break }
  ...
  for (var i = 0; i < tests.length; i++)
   if (! (typeof(field_val[tests[i]]) == 'function' ? field_val[tests[i]](value, field_val, form) : eval(field_val[tests[i]])))
    v_add_error(errors, field, tests[i], field_val, ifs_match, form);
 }
