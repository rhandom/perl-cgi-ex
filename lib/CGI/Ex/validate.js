// Copyright 2007 - Paul Seamons - $Revision: 1.77 $
...
function v_clean_val_hash (val_hash) {
 ...
 f = val_hash['group set_all_hook'];
 if (f && typeof(f) == 'string') eval("val_hash['group set_all_hook'] = "+f);
 f = val_hash['group clear_all_hook'];
 if (f && typeof(f) == 'string') eval("val_hash['group clear_all_hook'] = "+f);
...
function v_get_error_text (err, extra1, extra2) {
 ...
 var msg = v_find_val('error', extra1, extra2) || v_find_val(type + '_error', extra1, extra2);
 if (! msg) {
