    ...
    for (var i = 0; i < tests.length; i ++) {
      ...
      for (var j = 0; j < ref.length; j ++) {
        if (typeof(ref[j]) == 'function') {
          ...
          if (! (m = ref[j].match('^(!\\s*|)m([^\\s\\w])(.*)\\2([eigsmx]*)$')))
  ...
  if (typeof(val_hash) != 'object') {
    ...
    if (typeof(val_hash) == 'function') {
      ...
      // read hash from <input name=foo validation="">
        ...
        var found = 0;
        var _val_hash   = new Array();
        var group_order = new Array();
        var anchors     = new Array(); // hold anchors between yaml calls
        var ref;
        var str = form.getAttribute('validation');
        if (str && str.indexOf('\n') == -1)
          str = str.replace(new RegExp('(\\w+:\\ +(\'[^\']*\'|"[^"]*"|[^,:]+)),\\s+','g'),'$1\n');
        if (str && (ref = document.yaml_load(str, anchors)) && ref[0]) {
          _val_hash = ref[0];
          if (_val_hash.length) {
            alert("Inline validation may only have one group.");
            return;
          }
          found = 1;
        }
        ...
        for (var i = 0; i < form.elements.length; i ++) {
          ...
          var str  = form.elements[i].getAttribute('validation');
          if (! name || ! str) continue;
          str = str.replace('\\s*','');
          if (anc = str.match('^\\ *&(\\w+)\\ *')) {
            str = str.substring(anc[0].length);
            anc = anc[1];
          }
          if (str.indexOf('\n') == -1)
            str = str.replace(new RegExp('(\\w+:\\ +(\'[^\']*\'|"[^"]*"|[^,:]+)),\\s+','g'),'$1\n');
          var ref = document.yaml_load(str,anchors);
          if (! ref || ! ref[0]) continue;
          var hash = ref[0];
          if (! hash['field']) hash['field'] = name;
          if (anc) anchors[anc] = hash;
          _val_hash[name] = hash;
          group_order[group_order.length] = name;
          found = 1;
        }
        if (found) {
          val_hash = _val_hash;
          if (! val_hash['group order']) val_hash['group order'] = group_order;
        }
      }
    }
    if (typeof(val_hash) == 'string') {
