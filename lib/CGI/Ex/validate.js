// Copyright 2007 - Paul Seamons - $Revision: 1.51 $
...
function vob_check_conditional (form, ifs, N_level, ifs_match) {
 ...
 if (! ifs) return this.error("Need reference passed to check_conditional");
...
function vob_validate_buddy (form, field, field_val, N_level, ifs_match) {
 ...
 if (is_required) {
  ...
  if (! found) return this.add_error(errors, field, is_required, field_val, ifs_match, form);
 }

 if (field_val.min_values && n_values < field_val.min_values)
  return this.add_error(errors, field, 'min_values', field_val, ifs_match, form);

 if (typeof(field_val.max_values) == 'undefined') field_val.max_values = 1;
 if (field_val.max_values && n_values > field_val.max_values)
  return this.add_error(errors, field, 'max_values', field_val, ifs_match, form);
 ...
 for (var n = 0; n < values.length; n ++) {
  ...
  if (field_val.min_len && value.length < field_val.min_len) this.add_error(errors, field, 'min_len', field_val, ifs_match, form);
  if (field_val.max_len && value.length > field_val.max_len) this.add_error(errors, field, 'max_len', field_val, ifs_match, form);
 ...
 if (el.length && type != 'select-one') {
  ...
  for (var j=0;j<el.length;j++) {
   if (type.indexOf('multiple') != -1) {
    ...
    if (el[j].checked)  a.push(vob_get_form_value(el[j]));
   }
  }
