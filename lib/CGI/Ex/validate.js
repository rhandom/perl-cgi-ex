// Copyright 2007 - Paul Seamons - $Revision: 1.70 $
...
function v_clean_cond (ifs, N_level, ifs_match) {
 ...
 for (var i = 0; i < ifs.length; i++) {
  if (typeof(ifs[i]) == 'string') {
   if (ifs[i].match(/^\s*function\s*\(/)) ifs[i] = eval(ifs[i]);
...
function v_validate (form, val_hash) {
 ...
 for (var j = 0; j < fields.length; j++) {
  ...
  if (! chk[ref.field]) {
   chk[ref.field] = 1;
   val_hash['group was_checked'][ref.field] = 1;
   val_hash['group was_valid'][ref.field]   = 1;
   val_hash['group had_error'][ref.field]   = 0;
  }
  var err = v_validate_buddy(form, ref.field, ref, val_hash);
  if (err.length) {
   val_hash['group had_error'][ref.field] = 1;
   val_hash['group was_valid'][ref.field] = 0;
...
function v_validate_buddy (form, field, field_val, val_hash, ifs_match) {
 ...
 if (tests.length && ! needs_val) {
  if (field_val.vif_disable && val_hash['group was_valid'][field]) {
   var el = form[field]; if (! el.length) el = [el]; for (var i = 0; i < el.length; i++) el[i].disabled = true;
  }
  val_hash['group was_valid'][field] = 0;
  return [];
 }
 if (field_val.vif_disable) {
  var el = form[field]; if (! el.length) el = [el]; for (var i = 0; i < el.length; i++) el[i].disabled = false;
 }
 ...
 for (var i = 0; i < types.length; i++) {
  ...
  if (type.match(/^custom_js\d*$/)) {
   ...
   if (typeof(_fv) == 'function'
       ? ! _fv({'value':value, 'field_val':field_val, 'form':form, 'key':field_val.field, 'errors':errors})
...
function v_el_attach (el, fvs, form, val_hash) {
 ...
 var func = function () {
  ...
  for (var i = 0; i < fvs.length; i++) {
   var field_val = fvs[i];
   var k = field_val.delegate_error || field_val.field;
   if (! chk[k]) {
    chk[k] = 1;
...
function v_inline_error_clear (key, val_hash, form) {
   ...
   if (typeof(f) == 'function') if (f({'key':key, 'val_hash':val_hash, 'form':form, was_valid:val_hash['group was_valid'][key]})) return 1;
...
function v_inline_error_set (key, val, val_hash, form) {
   ...
   if (typeof(f) == 'function') if (f({'key':key, 'value':val, 'val_hash':val_hash, 'form':form, was_valid:val_hash['group was_valid'][key]})) return 1;
   var el = document.getElementById(key + v_find_val('as_hash_suffix', val_hash, '_error'));
