// Copyright 2007 - Paul Seamons - $Revision: 1.55 $
...
function v_get_error_text (err, extra1, extra2) {
 ...
 if (msg) {
  ...
  if (field_val[type + dig] && typeof(field_val[type + dig]) == 'string')
   msg = msg.replace(/\$value/g, field_val[type + dig]);
  return msg;
 }

 if (type == 'required' || type == 'required_if') return name + " is required.";
 if (type == 'match') return name + " contains invalid characters.";
 if (type == 'compare') return name + " did not fit comparison.";
 if (type == 'custom_js') return name + " did not match custom_js"+dig+" check.";
 if (type == 'enum') return name + " is not in the given list.";
 if (type == 'min_values') {
...
document.check_form = function (form, val_hash) {
 ...
 if (types.change || types.blur) {
  ...
  for (var i = 0; i < clean.fields.length; i++) {
   var field = clean.fields[i].field;
   var el = form[field];
   if (! el) return v_error("No form element by the name "+field);
   v_el_attach(el, clean.fields[i], form, val_hash);
  }
 }
...
function v_el_attach (el, field_val, form, val_hash) {
 if (! el.type) {
  if (el.length) for (var i = 0; i < el.length; i++) v_el_attach(el[i], field_val, form, val_hash);
  return;
 }
 var types = val_hash['group onevent'];
 var func = function () {
  var e = v_validate_buddy(form, field_val.field, field_val);
  if (! e || e == '') {
   var field = field_val.delegate_error || field_val.field;
   v_inline_error_clear(field, val_hash, form);
   return;
  }
  ...
  for (var key in e) {
   if (key == 'extend') continue; // Protoype Array()
   v_inline_error_set(key, e[key], val_hash, form);
  }
 }
 if (types.blur || field_val.onblur) el.onblur = func;
 if (types.change || field_val.onchange) {
  if (el.type.match(/(password|text|textarea)/)) el.onkeyup = func;
  if (el.type.match(/(checkbox|radio)/)) el.onclick = func;
 }
}
