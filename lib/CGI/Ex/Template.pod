...
=item Added HTML::Template::Expr vmethods
...
=item Added string comparison operators (gt ge lt le cmp)

    [% IF "a" lt "b" %]a is less[% END %]

=item Added numeric comparison operator (<=>)
...
Same as the fmt command.  Sprintfs the item with the passed arguement.
