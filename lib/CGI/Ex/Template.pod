...
compatibility with HTML::Template (HT) and HTML::Template::Expr (HTE).
Version 2.13 introduced the SYNTAX configuration allowing for
inclusion of TT style templates in HT and vice versa.  It also
provided the HTML::Template output and param methods which allow CET
to provide the HTML::Template interface.
...
non-XS version.  CET by itself is generally faster than TT, HTE, and
often HT.  The XS version is nearly always faster.  CET also uses less
...
now you can use them in TT templates as well.
...
=item Added INTERPOLATE capability
...
There is no test for numeric equality - well you could use [% ! (a <=> b) %].  This
may change in the future - I'd suggest using eq wherever you mean string equality.

=item C<!= ne>

Non associative binary.  Non-equality test.  TT chose to use Perl's ne for both
operators.  There is no test for numeric non-equality - well you could use [% (a <=> b) %].
This may change in the future - I'd suggest using ne wherever you mean string non-equality.
