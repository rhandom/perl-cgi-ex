...
The following seciton lists most of the publicly available methods.  Some less
...
The following is is the list of builtin virtual methods and filters
that can be called on scalar datatypes.  In CET and TT3, filters and
...
be rethrown using $error as the argument to THROW.
...
"|" operator (listed next).  Also - on objects the "." will always try and
call the method by that name.  To always call the vmethod - use "|".
...
Collapse adjacent whitepace to a single space.  The "=" is used to indicate CHOMP_COLLAPSE.
...
A list of templates to be processed before and prepended to the content
