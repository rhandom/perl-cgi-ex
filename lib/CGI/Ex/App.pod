...
that the following method is installed in your script.
    ...
    STEP: my_step
    FORM: {anything_else => "one%20two", bar => "three four"}
    WHY:  The third path_info_map regex matched.  Note that the
          %20 in bar was unescaped but the %20 in anything_else
          was not.  It is up to the application to URL decode
          such parameters as CGI::Ex::App doesn't decode parameters
          mapped from PATH_INFO.
