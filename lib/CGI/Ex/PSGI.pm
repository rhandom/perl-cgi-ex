...
CGI::Ex::PSGI - CGI utility suite for PSGI apps

=cut

###----------------------------------------------------------------###
#  Copyright 2013 - Paul Seamons                                     #
...
our $VERSION = '2.40';
our $PREFERRED_CGI_MODULE  ||= 'CGI::PSGI';

sub env { shift->{'env'} || die "Missing env\n" }
...
###----------------------------------------------------------------###
# similiar in nature to CGI::Ex subs but works with PSGI

sub mod_perl_version { 0 }
sub apache_request { undef }
sub is_mod_perl_1  { undef }
sub is_mod_perl_2  { undef }
...
sub content_typed { grep { $_ eq 'Content-Type' } keys %{{@{ $self->{'_headers'} || []}}} }
...
sub set_cookie {
    ...
    $args->{-path} ||= '/';
    $args->{-expires} = CGI::Ex::time_calc($args->{-expires}) if $args->{-expires};
    push @{ $self->{'_headers'} }, 'Set-Cookie' => "" . $self->object->cookie(%$args);
}
...
sub send_status {
    ...
    die "Cannot send a status ($code - $mesg) after content has been sent" if $self->content_typed;
    $self->{'_status'} = $code;
    push @{ $self->{'_body'} }, $msg;
...
=head1 CGI::Ex::PSGI SYNOPSIS

    ### You probably don't want to use CGI::Ex directly
    ### You probably should use CGI::Ex::App instead.

    my $cgix = CGI::Ex::PSGI->new;

    $cgix->print_content_type; # doesn't actually print yet
