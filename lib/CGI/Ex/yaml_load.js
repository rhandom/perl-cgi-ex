...
function yaml_parse (text) {
  ...
  while (! this.eoy) {
    ...
    this.hist += 'END OF DOCUMENT LOOP\n';
    if (this.hist.length > 10000) return this.error("Too much");
  }
...
function yaml_parse_throwaway () {
  this.hist += 'in yaml_parse_throwaway\n';
...
function yaml_parse_node (no_next) {
  ...
  this.hist += '(anchor: '+anchor+', alias: '+alias+', explicit: '+explicit+', implicit: '+implicit+')\n';
  ...
  this.hist += '(this.inline: '+this.inline+', preface: '+preface+', this.content: '+this.content+')\n';
  
  if (this.inline.length) {
    node = this.parse_inline(1, implicit, explicit, yclass);
    if (this.inline.length) return this.error("Parse error - must be single line");
    this.hist += 'NODEINLINE (this.offset[this.level]: '+this.offset[this.level]+', this.level: '+this.level+', this.indent: '+this.indent+')\n';
  } else {
    this.level ++;
    // block items
    if (indicator) {
      this.hist += 'PIPE (this.offset[this.level]: '+this.offset[this.level]+', this.level: '+this.level+', this.indent: '+this.indent+')\n';
      node = '';
      ...
      this.hist += 'NODE(this.offset[this.level]: '+this.offset[this.level]+', this.level: '+this.level+', this.indent: '+this.indent+')\n';
  ...
  this.hist += 'NODE OFFSET ('+this.offset+')\n';
  //  this.offset[this.offset.length - 1] = this.level;
  this.offset = this.offset.splice(0, this.level + 1);
  this.hist += 'NODE OFFSET ('+this.offset+')\n';
  this.hist += 'NODEEND (this.offset[this.level]: '+this.offset[this.level]+', this.level: '+this.level+', this.indent: '+this.indent+')\n';
  ...
  this.hist += 'out parse_node\n';
  return node;
}

function yaml_parse_next_line (type) {
  this.hist += 'in yaml_parse_next_line\n';
  var m;
  ...
  this.lines.shift();
  this.hist += 'LINE (this.lines[0]:'+this.lines[0]+')\n';
  this.line ++;
  this.eoy = this.done = ! this.lines.length;
  if (this.eoy) return;
  this.parse_throwaway();
  ...
  if (this.preface && (m = this.preface.match('[>\\|][+\\-]?(\\d*)\\s*$'))) {
    this.hist += 'NEXT (this.preface: '+this.preface+')\n';
  ...
  if (type == 2 && this.preface.match('^\\s*(!\\S*|&\\S+)*\\s*$')) {
    ...
    offset = this.offset[++ level];
    this.hist += 'NEXT(this.offset[this.level]: '+this.offset[this.level]+', this.level: '+this.level+', this.indent: '+this.indent+')\n';
  ...
  if (type == 1 && (m = this.lines[0].match('^ {'+offset+'}(.*)$'))) {
    ...
    this.hist += 'NEXT 2 ('+this.lines[0]+')\n';
    this.indent = offset;
    this.content = '';
  } else {
    this.hist += 'NEXT 3 ('+this.lines[0]+')\n';
    this.hist += 'NEXT1(this.offset[this.level]: '+this.offset[this.level]+', this.level: '+this.level+', this.indent: '+this.indent+')\n';
    m = this.lines[0].match('^( *)(\\S.*)$');
    ...
    this.indent  = len;
    this.content = m ? m[2] : '';
    this.hist += 'NEXT2(this.offset[this.level]: '+this.offset[this.level]+', this.level: '+this.level+', this.indent: '+this.indent+')\n';
  }
...
function yaml_parse_qualifiers (preface) {
  this.hist += 'in yaml_parse_qualifiers\n';
  var info = new Array();
  // 0 = anchor
  // 1 = alias
  // 2 = explicit
...
function yaml_parse_explicit (node, explicit) {
  this.hist += 'in yaml_parse_explicit\n';
  var m;
...
function yaml_parse_implicit (value) {
  this.hist += 'in yaml_parse_implicit\n';
  value.replace('\\s*$','');
  if (value == '') return '';
...
function yaml_parse_map (anchor) {
  ...
  while (! this.done && this.indent == this.offset[this.level]) {
    ...
    if (this.content.match('^?\\s*')) {
      ...
      this.hist += 'MAP (this.content: '+this.content+')\n';
    ...
    this.hist += 'MAP(key: '+key+')\n';
    var value = this.parse_node();
    this.hist += 'MAP(value: '+escape(value)+')\n';

    if (node[key]) this.warn('Warn - duplicate key '+key);
    else node[key] = value;

      this.hist += 'MAP(this.offset[this.level]: '+this.offset[this.level]+', this.level: '+this.level+', this.indent: '+this.indent+')\n';
  }

  this.hist += 'out yaml_parse_map\n';
  return node;
}

function yaml_parse_seq (anchor) {
  this.hist += 'in yaml_parse_seq\n';
  var m;
  var node = new Array ();
  if (anchor) this.anchors[anchor] = node;
  while (! this.done && this.indent == this.offset[this.level]) {
    ...
    this.hist += '(this.preface: '+this.preface+')\n';
    if (m = this.preface.match('^(\\s*)(\\w.*:( |$).*)$')) {
      this.hist += 'SEQ(this.offset[this.level]: '+this.offset[this.level]+', this.level: '+this.level+', this.indent: '+this.indent+')\n';
  ...
  this.hist += 'out yaml_parse_seq\n';
  return node;
}

function yaml_parse_inline (top, top_implicit, top_explicit, top_class) {
  this.hist += 'in yaml_parse_inline ('+this.inline+')\n';
  ...
  // simple
    ...
    if (top) {
      ...
      this.hist += 'INLINE (this.inline: '+this.inline+')\n';
      if (m = this.inline.match('^(|[^!@#%^&*].*?)(?=[,[\\]{}]|: |- |:\\s*$|$)')) {
